#-*- coding: utf-8 -*-
# =============================================================================
# One_pressurized_hole_ABAQUS_script_2D.py
#
# ABAQUS pipeline for a single pressurized cylindrical cavity in a 2D elastic
# medium. Used for:
#   1. Validating the FEM framework against the analytical single-cavity
#      solution (Appendix B of the paper), which gives closed-form expressions
#      for pressure, stretch, strain energy, and potential energy for a
#      neo-Hookean material.
#   2. Computing the reference potential energy PE₀ (potential energy of a
#      single isolated cavity), used to construct PE∞ = 2*PE₀.
#
# Problem setup:
#   - Single circular hole of radius R at the origin (0,0).
#   - Large circular domain of radius R_m approximates the infinite medium.
#   - Uniform pressure P applied to the hole boundary.
#   - Outer boundary is traction-free.
#   - Plane-strain conditions.
#
# The script follows the same three-section structure as
# Two_pressurized_holes_ABAQUS_script_2D.py:
#   1. Model creation and input file generation
#   2. Job submission
#   3. Strain energy and deformed coordinate extraction
#
# Reference:
...
#
# Creates a single ABAQUS model for one pressurized hole. Unlike the two-hole
# script, only one (circle[0] = 0, 0) geometry is used, and `circle_values`
# contains a single entry at the origin.
# =============================================================================

P  = 30            # Applied internal pressure
R_m, R = 5.0, 0.1  # Domain radius, hole radius

# Material model: change to 'LE', 'MR', or 'AB' as needed
material_model = 'NH'

# Mesh seeding sizes (same grading strategy as the two-hole script)
HoleSeed, MiddleSeed1, MiddleSeed2, OuterSeed = 0.002, 0.06, 0.12, 0.2

# Material properties (same definitions as in two-hole script)
...
for index, circle in enumerate(circle_values):
    ...
    # dR = 0 for the single-hole case (no second hole)
    dR       = 2 * circle[0] / R
    ...
    # --- Geometry: outer circle + single centered hole ---
    ...
    # --- Material and section (same as two-hole script) ---
    ...
    # --- Step: nlgeom=ON for nonlinear large deformation analysis ---
    mdb.models[model_name].StaticStep(
        adaptiveDampingRatio=0.05, continueDampingFactors=False, initialInc=0.1,
        name=step_name, nlgeom=ON, previous='Initial',
    ...
    # --- Pressure on single hole boundary ---
    # mask '[#1 ]' selects the single hole edge
    mdb.models[model_name].Pressure(
        ...
        region=Region(side1Edges=mdb.models[model_name].rootAssembly.instances[part_name].edges.getSequenceFromMask(mask=('[#1 ]',))))

    # --- Mesh: same partitioned, graded mesh as two-hole script ---
    ...
    mdb.models[model_name].sketches['__profile__'].CircleByCenterPerimeter(
        center=(0.0, 0.0), point1=(2.4, 0.0))
    mdb.models[model_name].sketches['__profile__'].CircleByCenterPerimeter(
        center=(0.0, 0.0), point1=(3.7, 0.0))
    ...
    mdb.models[model_name].parts[part_name].seedEdgeBySize(
        ...
        edges=mdb.models[model_name].parts[part_name].edges.getSequenceFromMask(mask=('[#8 ]',)),
        size=HoleSeed)
    mdb.models[model_name].parts[part_name].seedEdgeBySize(
        constraint=FINER, deviationFactor=0.1,
...
# Single dR value for the isolated-cavity case
dR_values = [(0.0)]
...
#
# Extracts strain energy and deformed cavity boundary coordinates for the
# single hole simulation. The logic is identical to the two-hole script,
# but only a single ODB is processed.
#
# Node IDs below are specific to the single-hole mesh. Node 4 is the
# reference center node; nodes 606 to 917 form the hole boundary circumference.
# Update these if the mesh parameters change.
# =============================================================================
...
for dR in dR_values:
    ...
    # Extract deformed coordinates of cavity boundary nodes
    node_ids = [4] + list(range(606, 918))
    ...
    for node_id in node_ids:
        ...
        displacement = np.append(displacement, 0.0)
