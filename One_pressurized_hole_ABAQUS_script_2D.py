...
R_m, R = 5.0, 0.1 # model radius, hole radius
...
for index, circle in enumerate(circle_values):
    ...
    mdb.models[model_name].Pressure(amplitude=UNSET, createStepName=step_name, distributionType=UNIFORM, field='', magnitude=P,
                                    name='InternalPressure', region=Region(side1Edges=mdb.models[model_name].rootAssembly.instances[part_name].edges.getSequenceFromMask(mask=('[#1 ]',))))
    ...
    mdb.models[model_name].parts[part_name].seedEdgeBySize(constraint=FINER, deviationFactor=0.1,
                                                           ...
                                                           size=MiddleSeed1)
    mdb.models[model_name].parts[part_name].seedEdgeBySize(constraint=FINER, deviationFactor=0.1,
                                                           edges=mdb.models[model_name].parts[
                                                               part_name].edges.getSequenceFromMask(mask=('[#2 ]',)),
                                                           size=MiddleSeed2)
    mdb.models[model_name].parts[part_name].seedEdgeBySize(constraint=FINER, deviationFactor=0.1,
...
for dR in dR_values:
    ...
    node_ids = [4] + list(range(606, 918))  # The range(start, stop) function generates numbers from start to stop-1
